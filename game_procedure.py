...
def vathmoi(cards, sum, name,whoisplaying,starting,row,column):
    ...
    if max < sum[i] :
        ...
        lmax = i
        if lmax = 1 :
        print ('Νικητής  είναι ο ' + name[lmax] + ' με σύνολο πόντων ' + max + '.')
        elif for i in len(lmax):
            print('Το παιχνίδι εληξε ισοπαλία ')
        #θελει συνεχεια,πιθανον ειναι λαθος το απο πανω
